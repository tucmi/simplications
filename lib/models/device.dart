...
class PrivacyAction {
  ...
  final String? deviceType; // Null = general action, specific deviceId = device-specific
...
class DeviceTemplate {
  ...
  const DeviceTemplate({
    ...
    this.deviceType = 'general', // speaker, camera, tv, thermostat, light, lock, etc.
    this.isCustom = false,
  });
}
...
class DeviceInstance {
  ...
  List<DeviceQuestion> get questions {
    ...
    if (template.deviceType == 'speaker') {
      q.addAll([
        const DeviceQuestion(
          id: 'voice_history',
          text: 'Können Sie Sprachaufnahmen aus der Geräte-Historie löschen oder diese Funktion deaktivieren?',
          hint: 'BSI-Empfehlung: Minimieren Sie die Speicherung von Aufnahmen oder löschen Sie sie regelmäßig.',
        ),
        const DeviceQuestion(
          id: 'voice_local',
          text: 'Wird die Sprachverarbeitung teilweise lokal auf dem Gerät durchgeführt?',
          hint: 'Lokale Verarbeitung reduziert die Menge an Daten, die in die Cloud übertragen werden.',
        ),
        const DeviceQuestion(
          id: 'skills_permissions',
          text: 'Überprüfen Sie regelmäßig, welche Fähigkeiten/Skills Zugriff auf Ihr Konto haben?',
          hint: 'Skills von Drittanbietern können sensible Daten abfangen. Nur notwendige Skills aktivieren.',
        ),
      ]);
      ...
      q.addAll([
        const DeviceQuestion(
          ...
          hint: 'BSI-Empfehlung: Verschlüsselte Verbindung verhindert Abhören und Datenklau unterwegs.',
        ),
        const DeviceQuestion(
          id: 'video_storage',
        ...
        const DeviceQuestion(
          ...
          hint: 'Sie sollten genau kontrollieren können, wer Zugang zu den Videoaufnahmen erhält.',
        ),
        const DeviceQuestion(
          id: 'motion_detection',
          text: 'Können Sie Bewegungserkennung deaktivieren oder zeitlich begrenzen, wenn Sie zu Hause sind?',
          hint: 'BSI-Empfehlung: Unnötige Aufnahmen vermeiden reduziert Datenmenge und Missbrauchsrisiko.',
        ),
      ]);
      ...
      q.addAll([
        ...
        const DeviceQuestion(
          ...
          text: 'Können Sie Werbe- und Tracking-Funktionen vollständig deaktivieren?',
          hint: 'Smart-TVs sammeln oft Daten über Ihre Sehgewohnheiten. Suchen Sie nach Datenschutzoptionen.',
        ),
        const DeviceQuestion(
          id: 'local_mode',
          text: 'Können Sie das Gerät über HDMI oder lokale Quellen ohne Internetverbindung nutzen?',
          hint: 'Offline-Betrieb schützt Ihre Nutzungsdaten vor Übertragung an den Hersteller.',
        ),
      ]);
      ...
      q.addAll([
        ...
        const DeviceQuestion(
          ...
          hint: 'Offline-Betrieb oder lokale Automatisierung schützt Ihre Daten vor Cloud-Zugriff.',
        ),
        const DeviceQuestion(
          id: 'family_access',
          text: 'Können Sie steuern, wer die Heizung anpassen darf (z. B. nur Gäste einschränken)?',
          hint: 'Haushaltsmitglieder sollten Kontrolle haben, ohne dass Besucher alles ändern können.',
        ),
      ]);
    } else if (template.deviceType == 'light' || template.deviceType == 'plug' || template.deviceType == 'blind') {
      q.addAll([
        const DeviceQuestion(
          id: 'local_control',
          text: 'Können Automatisierungen lokal ohne Internetverbindung erfolgen?',
          hint: 'Lokale Ausführung von Szenen und Zeitplänen schützt Ihre Privatsphäre besser.',
        ),
        const DeviceQuestion(
          id: 'usage_tracking',
        ...
        const DeviceQuestion(
          id: 'offline_fallback',
          text: 'Funktionieren die Geräte noch, wenn die Internet- oder App-Verbindung ausfällt?',
          hint: 'Zuverlässige lokale Funktion ist wichtig für alltägliche Nutzung und Privatsphäre.',
        ),
      ]);
      ...
      q.addAll([
        const DeviceQuestion(
          ...
          hint: 'Backup-Optionen verhindern Aussperrung bei Internet- oder Stromausfällen.',
        ),
        const DeviceQuestion(
          id: 'access_logging',
          text: 'Können Sie nachvollziehen, wer wann das Schloss geöffnet hat?',
          hint: 'Ein Zugriffsverlauf ermöglicht Ihnen, verdächtige Aktivitäten zu erkennen.',
        ),
        const DeviceQuestion(
          id: 'two_factor',
          text: 'Ist Zwei-Faktor-Authentifizierung (2FA) für das Sperr-Konto verfügbar?',
          hint: 'BSI-Empfehlung: 2FA schützt Ihr Konto vor unbefugtem Fernzugriff auf das Schloss.',
        ),
      ]);
      ...
      q.addAll([
        const DeviceQuestion(
          ...
          hint: 'Elternkontrolle sollte ermöglichen, Kontakte und Funktionen zu beschränken.',
        ),
        const DeviceQuestion(
          id: 'child_data_limits',
          text: 'Können Sie einschränken, welche persönlichen Daten das Spielzeug sammelt?',
          hint: 'Kinder-spezifische Geräte sollten minimale Datenmenge erfassen und speichern.',
        ),
        const DeviceQuestion(
          id: 'recording_disable',
          text: 'Können Sie Audio- und Videoaufnahmen auf dem Gerät deaktivieren?',
          hint: 'BSI-Empfehlung: Aufnahmen von Kindern sollten nur mit expliziter Kontrolle möglich sein.',
        ),
      ]);
      ...
      q.addAll([
        const DeviceQuestion(
          id: 'health_sharing',
          text: 'Können Sie steuern, welche Drittanbieter Zugriff auf Ihre Gesundheitsdaten haben?',
          hint: 'Gesundheitsdaten sind sensibel. Nur autorisierte Apps sollten Zugriff erhalten.',
        ),
        const DeviceQuestion(
          id: 'location_tracking',
          text: 'Können Sie Standortverfolgung deaktivieren, wenn Sie sie nicht benötigen?',
          hint: 'GPS-Tracking verbraucht Akku und kann Ihre Bewegungsmuster offenbaren.',
        ),
      ]);
    }

    if (template.hasCamera) {
      q.add(const DeviceQuestion(
        id: 'camera_consent',
    ...
    if (template.hasMicrophone) {
      q.add(const DeviceQuestion(
        ...
        hint: 'Smarte Lautsprecher und Geräte mit Mikrofonen können versehentlich aktiviert werden.',
      ));
    }
  ...
  int get riskScore {
    ...
    if (template.hasMicrophone && micDeactivatedWhenUnused == false) score += 10;
  ...
  List<PrivacyAction> get suggestedActions {
    ...
    if (passwordChanged == false) {
      actions.add(const PrivacyAction(
